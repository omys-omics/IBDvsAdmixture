...
do
  ...
  # run structure
  mkdir str.output/01.output.IBD.gen.$gen.200K.K2
  dos2unix $str
  structure_threader run -t 5 -i $str -o str.output/01.output.IBD.gen.$gen.200K.K2 --params mainparams -Klist 2 -R 5 -st ~/.local/bin/structure --log TRUE
done
